#!/usr/bin/env python3
"""
Visual test - shows where comment buttons are detected with red boxes
"""
...
print("=" * 60)
print("VISUAL COMMENT BUTTON DETECTION TEST")
print("=" * 60)
...
# Try to find comment buttons
ref_images = config.PIXEL_MATCHING['comment_button_images']
confidence = config.PIXEL_MATCHING['confidence']

print(f"\nSearching for comment buttons...")
...
output_path = output_dir / 'comment_detection_test.png'
screenshot.save(output_path)

print("\n" + "=" * 60)
if found_any:
    print("✓ COMMENT BUTTONS DETECTED!")
    print("=" * 60)
    print(f"\nRed boxes show detected comment buttons")
    print(f"Saved to: {output_path}")
    print("\nOpen the image to see where detection worked!")
    print("\nYou can now run: python3 main_feed_comment.py")
else:
    print("✗ NO COMMENT BUTTONS DETECTED")
    print("=" * 60)
    print(f"\nScreenshot saved to: {output_path}")
    print("\nThis shows what the script sees.")
    print("If you can see comment buttons in the image but none were detected,")
